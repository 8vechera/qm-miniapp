...
(function heroScroll() {
  ...
  frame.src = "lens.html?v=11";

  // Плавные кривые движения текста.
  const ease = t => t * t * (3 - 2 * t);                 // smoothstep (ease-in-out)
  // ПРОХОДЯЩИЙ текст: снизу -> центр -> вверх. У центра ПЛАВНО замедляется и снова разгоняется (без остановки).
  function passY(p, a, b, k) {                            // a..b — участок прогресса; k — сила торможения у центра (0..1)
    let t = (p - (a + b) / 2) / ((b - a) / 2);
    t = Math.max(-1, Math.min(1, t));
    return -100 * (k * t * t * t + (1 - k) * t);          // vh: +снизу, 0 центр, -сверху
  }
  // ПРИХОДЯЩИЙ и остающийся текст: снизу -> центр (замедляясь) и ОСТАЁТСЯ.
  function inY(p, start, center) {
    if (p <= start)  return 100;
    if (p >= center) return 0;
    return 100 * (1 - ease((p - start) / (center - start)));
  }
  ...
  function update() {
    ...
    // ЭКСПЕРИМЕНТ: в начале текста НЕТ. Текст идёт МЕДЛЕННО, у центра ПЛАВНО тормозит и снова разгоняется (без остановки).
    //  A (Продакшн полного цикла): проходит снизу -> центр(~0.39, замедляясь) -> вверх(разгоняясь).
    //  B (Ивенты/Подкасты/Live): приходит снизу, замедляется к центру(~0.85) и остаётся фоном.
    if (bgA) bgA.style.transform = `translateY(${passY(p, 0.06, 0.86, 0.65)}vh)`;   // медленнее (шире участок), центр ~0.46
    if (bgB) bgB.style.transform = `translateY(${inY(p, 0.68, 0.95)}vh)`;            // появляется ПОЗЖЕ (0.68→0.95) и остаётся
  }
  heroUpdate = update;
